...
| F0.3 | Compare backend options, document costs/risks, and prove Apple auth, authorization, atomic locks, realtime/APNs, backup/export, and environment isolation. The local PostgreSQL authorization/idempotency/atomic-lock/immutability slice now passes; hosted portions remain. Artifacts: evaluation, ADR-008 and redacted spike evidence. | Apple auth, hosted RLS/realtime/APNs, restore/export, environment isolation and true multi-session load pass; ADR explicitly accepted; no production credentials. | `in_progress` |
| F0.4 | Define vendor-neutral entities, ownership, state machines, OpenAPI, stable errors, idempotency, pagination, realtime reconciliation, audit events, and negative fixtures. | ADR direction accepted; schema review, API syntax/lint, fixture and generated-client compatibility evidence. | `in_progress` |
| F0.5 | Define data visibility/retention, identity/location privacy, consent, block/report, content policy, moderation, deletion/export, legal/store disclosures, and incident handling. | Product/safety/privacy owners approve; counsel review remains an external release gate. | `in_progress` |
...
| F0.7 | Provision isolated dev/QA/prod services and secrets; implement CI, contract/security/migration gates, content-free observability, promotion manifests, budget alerts, and rollback drills. The vendor-neutral runbook, environment validator and client-CI wrapper now exist and pass their local checks. | Accepted ADR and provider access; hosted CI, real-manifest isolation proof and successful rollback game day. | `in_progress` |
